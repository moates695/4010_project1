    ...
    for i = 1:data.n
        ...
        switch sensorID   
            case 1 % Lidar scan
                ...
                pairsGT = {};
                %disp(X);
                gtPose = ground(:,length(subsample_index));
                for q = 1:length(pairs)
                    pair = pairs{q};
                    %pairGCF = pair{1};
                    pairGCF = pair{2};
                    %pairsCCF{end+1} = inv(rotation(X(3)-pi/2))*(pairGCF-X(1:2));
                    pairsCCF{end+1} = inv(rotation(ground(3,length(subsample_index))-pi/2))*(pairGCF-ground(1:2,length(subsample_index)));
                    pairsGT{end+1} = pair{2}; 
                    %pairsGT{end+1} = inv(rotation(ground(3,length(subsample_index))-pi/2))*(pairGCF-X(1:2));
                    %disp(pairsCCF{end}); disp(pairsGT{end});
                end
                ...
                %disp(X);
                %lastX = X;
                [h, ~, ~] = fminsearch(@(h) CostFunction(h, gtPose(1:2), pairsCCF, pairsGT), double(X(3)), options);
                X(1:2) = gtPose(1:2);
                X(3) = h;
                %disp(X);
                %disp(lastX)
...
function cost = CostFunction(b, pose, ccfLandmarks, gtLandmarks) 
    estLandmarks = {};
    X = [pose(1); pose(2); b];
